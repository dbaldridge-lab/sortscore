...
To recreate this analysis, first run `demo_data/tiled_demo.ipynb` to generate the required input files.
...
The variant annotated as `multiple_aa` is a pathogenic control from the literature (identified by multiple synonymous edits in tiled data where the missense variant is not directly sequenced) and is excluded from this analysis.
...
```{r}
#| label: sortscore comparison
#| fig-width: 8
library(ggplot2)

corr <- cor.test(x=scores$sortscore_score, y=scores$effect, method='spearman', exact=FALSE)

# Add n and correlation to the plot
complete_scores <- scores[complete.cases(scores$sortscore_score, scores$effect),]
